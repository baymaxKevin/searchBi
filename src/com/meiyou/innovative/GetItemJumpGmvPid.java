...
@Resolve({"string,string->string,bigint,string,string,string,string,string,string,string,string"})
public class GetItemJumpGmvPid extends UDTF {
    ...
    public void process(Object[] args) throws UDFException {
        ...
        for(String parm : parms){
            if(parm.contains("myuid")||parm.contains("userid")){
                ...
                String[] bids = parm.split("=");
                if(bids.length > 1){
                    brand_area_id = bids[1];
                }
            }
        }
        forward(access_time,uid,iid,path,app_id,platform,action,brand_area_id,pid,version);
    }
